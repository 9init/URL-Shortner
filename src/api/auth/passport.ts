...
passport.use("stateless", new CustomStrategy(async (req, done)=>{
    ...
    try {
        ...
        if (!success) return done(null, null) // return custom error message again
            return done(null, user) // Correct information
    } catch (error) {
        if (error) return done(error, null) // return error message
    }
...
function failureLogin(req: express.Request, res: express.Response){
    res.status(403).json({error: "Wrong Info"})
}

const loginHandler = passport.authenticate("stateless", {successRedirect: "/home", failureRedirect: "/login"})
