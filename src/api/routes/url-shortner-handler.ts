...
router.post("/shorten", async (req: express.Request, res: express.Response)=>{
    ...
    let urlShortenBody: ShortLinkInput = {
        ...
        shortUrl: shortUrl,
        longUrl: longUrl,
        username: req.isAuthenticated() ? (req.user as UserOuput).username : req.socket.remoteAddress
    }
 ...
 */
 router.get("/shortlinks", async (req: express.Request, res: express.Response)=>{
    try {
        let data : {[key: string]: ShortLinkOuput[]} = {}
        // get all records
        let shortUrls = await UrlDal.getAll()
        shortUrls.forEach((v)=>{
            if(!data[v.username]) data[v!.username] = []
            if(v.shortUrl) data[v.username].push(v)
        })
